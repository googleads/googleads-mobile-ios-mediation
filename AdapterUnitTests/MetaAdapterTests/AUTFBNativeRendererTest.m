...
@implementation AUTFBNativeRendererTest {
  ...
  id _mockMediaView;
  // We use id because this delegate can be either an FBNativeAdDelegate or an FBNativeBannerAdDelegate.
  // The __weak reference is used here to verify that the code under test (the GADFBNativeRenderer)
  // correctly retains its delegate. If the delegate is not retained by the GADFBNativeRenderer,
  // this weak reference would become nil, revealing a retention bug in the adapter.
...
- (void)setUp {
  ...
  OCMStub([_mockFBNativeAdBase nativeAdWithPlacementId:[OCMArg any]
      ...
      .andDo(^(NSInvocation *invocation) {
        id resultAd =
            self->_shouldReturnNativeBannerAd ? self->_mockFBNativeBannerAd : self->_mockFBNativeAd;
...
- (void)testNativeAdMediaFullscreenEnterandExit {
  OCMStub([(FBNativeAd *)_mockFBNativeAd loadAdWithBidPayload:AUTKNativeBidResponse])
      .andDo(^(NSInvocation *invocation) {
        FBNativeAd *ad = (FBNativeAd *)invocation.target;
        [self->_nativeAdDelegate nativeAdDidLoad:ad];
  ...
  [(id<FBMediaViewDelegate>)nativeAd
      mediaViewWillEnterFullscreen:(FBMediaView *)nativeAd.mediaView];
...
- (void)testLoadNativeAdWhenAgeRestrictedTreatmentIsTeen {
  GADMobileAds.sharedInstance.requestConfiguration.ageRestrictedTreatment =
...
- (void)testLoadNativeBannerAdWhenAgeRestrictedTreatmentIsTeen {
  _shouldReturnNativeBannerAd = YES;
  GADMobileAds.sharedInstance.requestConfiguration.ageRestrictedTreatment =
      GADAgeRestrictedTreatmentTeen;
  OCMExpect([_fbAdSettingsMock setMixedAudience:NO]);
