...
- (void)loadNativeAdForAdConfiguration:(nonnull GADMediationNativeAdConfiguration *)adConfiguration completionHandler:(nonnull GADMediationNativeLoadCompletionHandler)completionHandler {
    ...
    _adUnitId = adUnitId;
    _nativeManager = [[MTGNativeAdManager alloc]initWithPlacementId:placementId unitID:adUnitId supportedTemplates:@[[MTGTemplate templateWithType:MTGAD_TEMPLATE_BIG_IMAGE adsNum:1]] autoCacheImage:YES adCategory:MTGAD_CATEGORY_ALL presentingViewController:rootViewController];
...
- (void)nativeAdsLoaded:(nullable NSArray *)nativeAds nativeManager:(nonnull MTGNativeAdManager *)nativeManager {
    if ([nativeAds isKindOfClass:NSArray.class] && nativeAds.count > 0) {
        ...
        NSError *error =
        GADMAdapterMintegralErrorWithCodeAndDescription(GADMintegralErrorAdNotAvailable, @"Mintegral SDK failed to return a waterfall native ad.");
