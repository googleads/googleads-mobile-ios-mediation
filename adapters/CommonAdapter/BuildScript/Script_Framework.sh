...
createFramework() {
  ...
  # Create Headers directory and copy CommonAdapter-Swift.h file
  mkdir -p "${TEMP_FRAMEWORK_LOCATION}/Headers/"
  /bin/cp -a "${OBJROOT}/${1}/CommonAdapter.build/${CONFIGURATION}-${1}/Adapter.build/DerivedSources/CommonAdapter-Swift.h" "${TEMP_FRAMEWORK_LOCATION}/Headers/"
  ...
  xcodebuild -target CommonAdapterFramework \
  -configuration "${CONFIGURATION}" \
  -sdk "${1}" \
  ARCHS="${2}" \
