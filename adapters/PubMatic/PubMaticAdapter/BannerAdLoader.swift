...
final class BannerAdLoader: NSObject, MediationBannerAd, @unchecked Sendable {
  ...
  func loadAd() {
    guard let bidResponse = adConfiguration.bidResponse, let watermark = adConfiguration.watermark else {
      handleLoadedAd(
        nil,
    ...
    DispatchQueue.main.async { [weak self] in
      ...
      self.client.loadRtbBannerView(bidResponse: bidResponse, delegate: self, watermarkData: watermark)
    }
  }
