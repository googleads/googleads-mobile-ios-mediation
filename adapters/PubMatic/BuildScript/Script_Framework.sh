...
createFramework() {
  ...
  # Create Headers directory and copy PubMaticAdapter-Swift.h file
  mkdir -p "${TEMP_FRAMEWORK_LOCATION}/Headers/"
  /bin/cp -a "${OBJROOT}/${1}/PubMaticAdapter.build/${CONFIGURATION}-${1}/Adapter.build/DerivedSources/PubMaticAdapter-Swift.h" "${TEMP_FRAMEWORK_LOCATION}/Headers/"
