...
- (void)initializeAdColonyWithAppId:(NSString *)appId zones:(NSArray *)newZones request:(id<GADMediationAdRequest>)request callback:(void(^)())callback {
    @synchronized (self) {
        NSLogDebug(@"new zones: %@", newZones);
        NSLogDebug(@"old zones: %@", self.zones);
        
        // Even if ADC configure should be smart with configuring with superset/subset of zones, manage it here too
        ...
        if (self.initState == INIT_STATE_INITIALIZED) {
            ...
            if (self.initState != INIT_STATE_INITIALIZING) {
                ...
                [AdColony configureWithAppID:appId zoneIDs:[self.zones allObjects] options:options completion:^(NSArray<AdColonyZone *> * _Nonnull zones) {
                ...
                //NSLog(@"AdColonyAdapter [*Warning*] : configuration called while another in flight, 2nd call ignored.");
            }
        }
    }
}
...
@property (weak) id<GADMediationAdRequest> request; // this is shortcut to either connector or rewardConnector, whichever is valid
...
- (void)setUp {
    if (self.appId) {
        ...
        NSError *error = [NSError errorWithDomain:kGADErrorDomain code:kGADErrorMediationAdapterError userInfo:@{NSLocalizedDescriptionKey: @"Adapter not initialized"}];
...
- (NSArray *)parseZoneIDs:(NSString *)zoneList {
    ...
    for (NSString *zoneID in zoneIDs) {
        NSString *trimmed = [zoneID stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
...
- (void)setupZoneFromRequest:(id<GADMediationAdRequest>)request callback:(void(^)(NSString *zone))callback {
    NSDictionary *credentials = [request credentials];
    
    // Support arrays for older implementations, they won't have to change their zones on the dashboard
    ...
    [[AdColonyInitializer sharedInstance] initializeAdColonyWithAppId:self.appId zones:@[zone] request:request callback:^{
        if (callback) {
            callback(zone);
        }
    }];
...
- (void)getInterstitialFromZoneId:(NSString *)zone withRequest:(id<GADMediationAdRequest>)request {
    ...
    [AdColony requestInterstitialInZone:zone options:options success:^(AdColonyInterstitial * _Nonnull ad) {
        ...
        if (weakSelf.connector) {
            ...
            if (zone.rewarded) {
                ...
                NSLog(@"AdColonyAdapter [**Error**] : Zone used for rewarded video is not a rewarded video zone on AdColony portal.");
                NSError *error = [NSError errorWithDomain:kGADErrorDomain code:kGADErrorInvalidRequest userInfo:@{NSLocalizedDescriptionKey: @"Zone used for rewarded video is not a rewarded video zone on AdColony portal"}];
        ...
        // 1. Admob interstitial request from zone A. Causes ADC configure to occur with zone A, then ADC ad request from zone A. Both succeed.
        // 2. Admob rewarded video request from zone B. Causes ADC configure to occur with zones A, B, then ADC ad request from zone B. Both succeed.
        // 3. Try to present ad loaded from zone A. It doesn’t show because of error: `No session with id: xyz has been registered. Cannot show interstitial.`
        ...
        NSError *error = [NSError errorWithDomain:kGADErrorDomain code:kGADErrorInvalidRequest userInfo:@{NSLocalizedDescriptionKey: err.localizedDescription}];
        if (weakSelf.connector) {
...
- (void)requestRewardBasedVideoAd {
    // The only difference between Interstitials and RewardedVideo is within the interstitial and zone returned from the API
    // Rewarded videos from admob only initializes once, need to get zone from the request every time, interstitials are instantiated every time
...
- (void)presentInterstitialFromRootViewController:(UIViewController *)rootViewController {
    ...
    if (self.rewardConnector) {
        ...
        [zone setReward:^(BOOL success, NSString * _Nonnull name, int amount) {
            GADAdReward *reward = [[GADAdReward alloc] initWithRewardType:name rewardAmount:(NSDecimalNumber *)[NSDecimalNumber numberWithInt:amount]];
...
- (void)getBannerWithSize:(GADAdSize)adSize {
    NSError *error = [NSError errorWithDomain:kGADErrorDomain code:kGADErrorInvalidRequest userInfo:@{NSLocalizedDescriptionKey: @"AdColony adapter doesn't currently support Instant-Feed videos."}];
...
- (void)stopBeingDelegate {
    // AdColony retains the AdColonyAdDelegate during ad playback and does not issue any callbacks outside of ad playback or async calls already in flight
