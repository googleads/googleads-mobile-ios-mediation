...
- (void)loadAd {
    ...
    [GADMAdapterAdColonyHelper
     ...
     callback:^(NSString *zone, NSError *error) {
        ...
        UIViewController *viewController = strongSelf->_adConfiguration.topViewController;
        if (!viewController) {
            NSString *errorMessage = @"View controller cannot be nil.";
            NSError *error =
                GADMAdapterAdColonyErrorWithCodeAndDescription(GADMAdapterAdColonyErrorRootViewControllerNil, errorMessage);
            strongSelf->_renderCompletionHandler(nil, error);
            return;
        }
        AdColonyAdSize adColonyAdSize = AdColonyAdSizeMake(adSize.size.width, adSize.size.height);
        [AdColony requestAdViewInZone:zone withSize:adColonyAdSize andOptions:options viewController:viewController andDelegate:self];
    }];
}
