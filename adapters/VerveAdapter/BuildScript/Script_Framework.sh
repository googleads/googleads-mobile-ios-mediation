...
createFramework() {
  ...
  # Create Headers directory and copy VerveAdapter-Swift.h file
  mkdir -p "${TEMP_FRAMEWORK_LOCATION}/Headers/"
  /bin/cp -a "${OBJROOT}/${1}/VerveAdapter.build/${CONFIGURATION}-${1}/Adapter.build/DerivedSources/VerveAdapter-Swift.h" "${TEMP_FRAMEWORK_LOCATION}/Headers/"
  ...
  xcodebuild -target VerveAdapterFramework \
  -configuration "${CONFIGURATION}" \
  -sdk "${1}" \
  ARCHS="${2}" \
