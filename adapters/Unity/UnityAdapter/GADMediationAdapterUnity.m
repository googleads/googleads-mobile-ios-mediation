...
+ (void)updatePrivacyPreferences {
  ...
  // Non-Behavioral / Restricted (Non-behavioral = YES)
  // We restrict data if:
  // - TFCD is YES
  // - TFUA is YES
  // - Both TFCD and TFUA are nil
  if ([tagForChildDirectedTreatment isEqual:@1] || [tagForUnderAgeOfConsent isEqual:@1] ||
      (tagForChildDirectedTreatment == nil && tagForUnderAgeOfConsent == nil)) {
    [userMetaData set:@"user.nonbehavioral" value:@YES];
  }
  // BEHAVIORAL Logic (Non-behavioral = NO)
  // We have at least one explicit "Adult" signal without a conflicting Child
  // signal.
  else {
