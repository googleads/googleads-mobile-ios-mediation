...
@interface GADMAdapterUnityWeakReference ()
...
@implementation GADMAdapterUnityWeakReference

+ (BOOL)set:(NSSet *)set containsObject:(id)anObject {
  return [set containsObject:[[GADMAdapterUnityWeakReference alloc] initWithObject:anObject]];
}

- (instancetype)init {
  @throw [NSException exceptionWithName:@"Weak reference initialization"
                                 reason:@"Use initWithObject:, not init"
                               userInfo:nil];
}
...
- (BOOL)isEqual:(id)anObject {
  if ([anObject isKindOfClass:[GADMAdapterUnityWeakReference class]]) {
