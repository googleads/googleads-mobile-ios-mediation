...
- (void)getInterstitial {
  ...
  NSString *log = [NSString
      stringWithFormat:@"getInterstitial params: appKey=%@, is testing enabled=%d, instance id: %@",
...
- (void)presentInterstitialFromRootViewController:(UIViewController *)rootViewController {
  [self
      onLog:[NSString stringWithFormat:@"presentInterstitialFromRootViewController for instance %@",
                                       self.instanceId]];
...
- (void)interstitialDidFailToLoadWithError:(NSError *)error instanceId:(NSString *)instanceId {
  NSString *log =
      [NSString stringWithFormat:@"interstitialDidFailToLoadWithError: %@, for instance: %@",
  ...
  if (!error) {
    error = [self
        createErrorWith:@"network load error"
          ...
          andSuggestion:
              @"Please check that your network configuration are according to the documentation."];
  }
...
- (void)interstitialDidFailToShowWithError:(NSError *)error instanceId:(NSString *)instanceId {
  [self
      onLog:[NSString stringWithFormat:@"interstitialDidFailToShowWithError: %@, for instance: %@",
                                       error.localizedDescription, instanceId]];
