  ...
  if (![self isEmpty:applicationKey]) {
    ...
    NSString *log =
        [NSString stringWithFormat:
                      @"rewarded setUp params: appKey=%@, is testing enabled=%d, instance id: %@",
                      applicationKey, self.isLogEnabled, self.instanceId];
    [self onLog:log];
...
- (void)requestRewardBasedVideoAd {
  ...
  if ([IronSource hasISDemandOnlyRewardedVideo:self.instanceId]) {
    [self
        onLog:[NSString stringWithFormat:@"reward based video ad is available for instance is: %@",
                                         self.instanceId]];
...
- (void)rewardedVideoHasChangedAvailability:(BOOL)available instanceId:(NSString *)instanceId {
  [self onLog:[NSString stringWithFormat:@"rewardedVideoHasChangedAvailability %@ for instance %@ ",
                                         available ? @"TRUE" : @"FALSE", instanceId]];
  ...
  if (available) {
    ...
    NSError *AvailableAdsError =
        [self createErrorWith:@"IronSource network isn't available"
                ...
                andSuggestion:@"Please talk with your PM and check that your network configuration "
                @"are according to the documentation."];
    [strongConnector adapter:self didFailToLoadRewardBasedVideoAdwithError:AvailableAdsError];
  }
}

/// Invoked when the user completed the video and should be rewarded.
/// If using server-to-server callbacks you may ignore these events and wait for the callback from
/// the IronSource server.
/// |placementInfo| - IronSourcePlacementInfo - an object contains the placement's reward name and
/// amount
- (void)didReceiveRewardForPlacement:(ISPlacementInfo *)placementInfo
                          instanceId:(NSString *)instanceId {
  [self onLog:[NSString
                  stringWithFormat:@"didReceiveRewardForPlacement for instance: %@", instanceId]];
...
/// |error| - NSError which contains the reason for the failure. The error contains error.code and
/// error.localizedDescription.
- (void)rewardedVideoDidFailToShowWithError:(NSError *)error instanceId:(NSString *)instanceId {
  [self
      onLog:[NSString stringWithFormat:@"rewardedVideoDidFailToShowWithError: %@, for instance: %@",
