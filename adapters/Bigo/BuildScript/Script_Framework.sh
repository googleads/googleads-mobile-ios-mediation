...
createFramework() {
  ...
  # Create Headers directory and copy BigoAdapter-Swift.h file
  mkdir -p "${TEMP_FRAMEWORK_LOCATION}/Headers/"
  /bin/cp -a "${OBJROOT}/${1}/BigoAdapter.build/${CONFIGURATION}-${1}/Adapter.build/DerivedSources/BigoAdapter-Swift.h" "${TEMP_FRAMEWORK_LOCATION}/Headers/"
  ...
  xcodebuild -target BigoAdapterFramework \
  -configuration "${CONFIGURATION}" \
  -sdk "${1}" \
  ARCHS="${2}" \
