...
- (void)enqueue:(id)item
{
    @synchronized ( self.lock )
    {
        ...
        {
            [NSException raise: NSInvalidArgumentException format: @"[%s]: attempt to insert nil value", __PRETTY_FUNCTION__];
        }
    }
}
...
- (nullable id)objectAtIndex:(NSUInteger)index
{
    @synchronized ( self.lock )
    {
        ...
        {
            [NSException raise: NSRangeException format: @"[%s]: attempt to index stack of size %lu at index %lu", __PRETTY_FUNCTION__, (unsigned long)self.count, (unsigned long)index];
            return nil;
        }
    }
}
