...
public class GADMWaterfallAppLovinBannerRenderer: NSObject, MediationBannerAd {
  ...
  private let client: AppLovinClient
  ...
  nonisolated(unsafe) private var adView: UIView?
  ...
  @objc public init(adConfiguration: MediationBannerAdConfiguration) {
    ...
    self.client = AppLovinClientFactory.createClient()
    super.init()
  }
  ...
  @objc public func loadAd(
    ...
    client.loadBannerAd(
      for: sharedALSdk,
      size: appLovinAdSize,
      zoneIdentifier: zoneIdentifier.isEmpty ? nil : zoneIdentifier,
      delegate: appLovinDelegate
    )

    self.adView = client.bannerAdView
    if let bannerView = self.adView {
  ...
  nonisolated public var view: UIView {
    return (self.adView as UIView?) ?? dummyView
  }
