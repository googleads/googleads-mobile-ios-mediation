...
public final class GADMediationAdapterAppLovin: NSObject, @preconcurrency RTBAdapter {
  ...
  @objc public static func setUp(
    ...
    if sdkKeys.count > 1 {
      GADMAdapterAppLovinUtils.log(
        "More than one SDK key was found. The adapter will use \(sdkKey) to initialize the "
          + "AppLovin SDK."
      )
    }
  ...
  // Note: Banner ads are supported by AppLovin only for Waterfall and not for Bidding.
  // So, all banner ad load requests are assumed to be for Waterfall.
