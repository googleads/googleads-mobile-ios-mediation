...
- (void)getInterstitial
{
    ...
    if ( ALGADAdsQueue.count < ALGADAdsQueueMinCapacity )
    {
        ...
        //    [NSTimer scheduledTimerWithTimeInterval: 10.0 target: self selector: @selector(loadAfterSleep:) userInfo: nil repeats: NO];
    }
    else
    {
        [self log: @"Attempting to get another interstitial ad when we have preloaded ads already for placement: %@", self.placement];
...
- (void)presentInterstitialFromRootViewController:(UIViewController *)rootViewController
{
    ...
    if ( dequeuedAd )
    {
        ...
        [self.interstitial showOver: [UIApplication sharedApplication].keyWindow placement: self.placement andRender: dequeuedAd];
    }
    else
    {
...
- (void)getBannerWithSize:(GADAdSize)adSize
{
    NSError *error = [NSError errorWithDomain: kGADMAdapterAppLovinErrorDomain
                                     ...
                                     userInfo: @{ NSLocalizedDescriptionKey : @"Banners are not supported by this adapter"}];
