...
protocol OpenWrapSDKClient: NSObject {
  ...
  /// Load a banner ad.
  @MainActor func loadRtbBannerView(bidResponse: String, delegate: POBBannerViewDelegate)

  /// Load an interstitial ad.
  func loadRtbInterstitial(bidResponse: String, delegate: POBInterstitialDelegate)

  /// Load a rewarded ad.
  func loadRtbRewardedAd(bidResponse: String, delegate: POBRewardedAdDelegate)
...
final class OpenWrapSDKClientImpl: NSObject, OpenWrapSDKClient {
  ...
  func collectSignals(for adFormat: POBAdFormat) -> String {
    ...
    // TODO(hikaruwatanabe): update gpid property.
    config.gpid = nil
    // TODO(hikaruwatanabe): update the host to admob from unknown once OpenWrapSDK supports it.
    return POBSignalGenerator.generateSignal(for: .unknown, andConfig: config)
  }

  @MainActor
  func loadRtbBannerView(bidResponse: String, delegate: POBBannerViewDelegate) {
    bannerView = POBBannerView()
    bannerView?.delegate = delegate
    bannerView?.pauseAutoRefresh()
    bannerView?.loadAd(withResponse: bidResponse)
  }
  ...
  func loadRtbInterstitial(
    ...
    interstitial?.loadAd(withResponse: bidResponse)
  }

  func loadRtbRewardedAd(bidResponse: String, delegate: any POBRewardedAdDelegate) {
    rewardedAd = POBRewardedAd()
    rewardedAd?.delegate = delegate
    rewardedAd?.load(withResponse: bidResponse)
  }
