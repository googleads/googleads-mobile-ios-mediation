...
- (void)getBannerWithSize:(GADAdSize)adSize {
    ...
    NSString *description = [NSString stringWithFormat:@"%@ is not supported bunner", self.class];
    [self.interstitialAdConnector adapter:self didFailAd:[GADMMaioError errorWithDescription:description]];
}
...
- (void)getInterstitial {
    ...
    if (![repository isInitializedWithMediaId:self.mediaId]) {
        ...
        [repository addMaioInstance: [Maio startWithNonDefaultMediaId:self.mediaId delegate:[GADMMaioDelegateAggregate sharedInstance]]];
        return;
    }
...
- (void)maioDidInitialize {
    [[GADMMaioMaioInstanceRepository new] setInitialized:YES mediaId:self.mediaId];
}
...
- (void)maioDidChangeCanShow:(NSString *)zoneId newValue:(BOOL)newValue{
    if (self.zoneId && ! [self.zoneId isEqualToString:zoneId] ) return;
    if (! newValue) return;
