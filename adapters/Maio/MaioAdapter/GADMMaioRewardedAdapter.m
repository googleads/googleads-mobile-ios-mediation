...
- (void)setUp {
    GADMMaioMaioInstanceRepository *repository = [GADMMaioMaioInstanceRepository new];
    // Custom Event パラメータ（mediaId, zoneId）をロード。
    GADMMaioParameter *parameter = [self.class loadCustomEventParametersServerFromConnector:_rewardBasedVideoAdConnector];
    ...
    if (![repository isInitializedWithMediaId:_mediaId]) {
        ...
        [repository addMaioInstance: [Maio startWithNonDefaultMediaId:_mediaId delegate:[GADMMaioDelegateAggregate sharedInstance]]];
    } else {
...
- (void)requestRewardBasedVideoAd {
    ...
    if ([repository isInitializedWithMediaId:_mediaId]) {
        // ゾーンID が変更（直前とは異なる AdUnitID を使用）されるケースがあるので、Custom Event パラメータ（mediaId, zoneId）を再ロード。
        GADMMaioParameter *parameter = [self.class loadCustomEventParametersServerFromConnector:_rewardBasedVideoAdConnector];
        _zoneId = parameter.zoneId;
        ...
        if ([maioInstance canShowAtZoneId:_zoneId]) {
            ...
            NSString *description = [NSString stringWithFormat:@"%@ failed to receive reward based video ad.", NSStringFromClass([Maio class])];
...
- (void)presentRewardBasedVideoAdWithRootViewController:(UIViewController *)viewController {
    ...
    // GADMAdapterUnity.m の実装に倣って、ここで adapterDidOpenRewardBasedVideoAd を呼ぶ。
...
- (void)maioDidChangeCanShow:(NSString *)zoneId newValue:(BOOL)newValue {
    if(!newValue) return;
    if (_zoneId && ! [_zoneId isEqualToString: zoneId]) return;
...
- (void)maioDidFinishAd:(NSString *)zoneId playtime:(NSInteger)playtime skipped:(BOOL)skipped rewardParam:(NSString *)rewardParam {
    ...
    if (!skipped) {
        GADAdReward *reward = [[GADAdReward alloc] initWithRewardType:rewardParam ?: @"" rewardAmount:[NSDecimalNumber one]];
...
- (void)maioDidFail:(NSString *)zoneId reason:(MaioFailReason)reason {
    ...
    [self notifyThatDidFailToLoadAdWwithDescription:[GADMMaioError stringFromFailReason:reason]];
}
...
- (void)notifyThatDidFailToLoadAdWwithDescription:(NSString *)description {
    if (_isLoading) {
        NSError *errorWithDescription = [GADMMaioError errorWithDescription:description];
        if (_rewardBasedVideoAdConnector) {
            [_rewardBasedVideoAdConnector adapter:self didFailToLoadRewardBasedVideoAdwithError:errorWithDescription];
        }
        _isLoading = NO;
    }
}
