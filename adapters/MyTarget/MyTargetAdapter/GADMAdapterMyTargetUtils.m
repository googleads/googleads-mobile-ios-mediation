...
+ (NSError *)errorWithDescription:(NSString *)description
{
	...
	NSError *error = [NSError errorWithDomain:kGADMAdapterMyTargetErrorDomain code:1000 userInfo:userInfo];
	return error;
}
...
+ (void)fillCustomParams:(MTRGCustomParams *)customParams withConnector:(id<GADMediationAdRequest>)connector
{
...
+ (BOOL)isSize:(GADAdSize)size1 equalToSize:(GADAdSize)size2
{
	...
	return ceilf(size1.size.width) == ceilf(size2.size.width) && ceilf(size1.size.height) == ceilf(size2.size.height);
}
