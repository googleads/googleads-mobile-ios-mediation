...
+ (nullable id<GADMediatedUnifiedNativeAd>)mediatedUnifiedNativeAdWithNativePromoBanner:(MTRGNativePromoBanner *)promoBanner
                                                                               nativeAd:(MTRGNativeAd *)nativeAd
                                                                         autoLoadImages:(BOOL)autoLoadImages
                                                                            mediaAdView:(MTRGMediaAdView *)mediaAdView {
  guard(promoBanner.title && promoBanner.descriptionText && promoBanner.image && promoBanner.ctaText) else return nil;
...
- (nullable id<GADMediatedUnifiedNativeAd>)initWithNativePromoBanner:(MTRGNativePromoBanner *)promoBanner
                                                            nativeAd:(MTRGNativeAd *)nativeAd
                                                      autoLoadImages:(BOOL)autoLoadImages
                                                         mediaAdView:(MTRGMediaAdView *)mediaAdView {
  self = [super init];
  if (self) {
    ...
    if (promoBanner) {
      ...
      GADNativeAdImage *image = [GADMAdapterMyTargetUtils nativeAdImageWithImageData:promoBanner.image];
...
- (void)didRenderInView:(UIView *)view
 ...
 nonclickableAssetViews:(NSDictionary<GADUnifiedNativeAssetIdentifier, UIView *> *)nonclickableAssetViews
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [self.nativeAd registerView:view withController:viewController withClickableViews:clickableAssetViews.allValues];
  });
}
