...
NSString *const kGADMAdapterTapjoyVersion = @"11.9.1.0";
...
- (void)initRVPlacement {
    ...
    _rvPlacement = [TJPlacement placementWithName:videoPlacementName mediationAgent:kMediationAgent mediationId:nil delegate:self];
...
- (void)initInterstitialPlacement {
    NSString *interstitialPlacementName = [[_interstitialConnector credentials] objectForKey:@"placementName"];
    _intPlacement = [TJPlacement placementWithName:interstitialPlacementName mediationAgent:kMediationAgent mediationId:nil delegate:self];
...
- (void)requestDidSucceed:(TJPlacement*)placement{
    if (!placement.contentAvailable) {
        if (_rewardBasedVideoAdConnector) {
            ...
            _intIsRequesting = NO;
            NSError *adapterError = [NSError errorWithDomain:@"com.google.mediation.tapjoy" code:0 userInfo:@{NSLocalizedDescriptionKey: @"Tapjoy interstitial not available"}];
            [_interstitialConnector adapter:self didFailAd:adapterError];
        }
    }
}
...
- (void)requestDidFail:(TJPlacement*)placement error:(NSError*)error{
    if (_rewardBasedVideoAdConnector) {
        ...
        _intIsRequesting = NO;
        NSError *adapterError = [NSError errorWithDomain:@"com.google.mediation.tapjoy" code:0 userInfo:@{NSLocalizedDescriptionKey: @"Tapjoy interstitial failed to load"}];
        [_interstitialConnector adapter:self didFailAd:adapterError];
