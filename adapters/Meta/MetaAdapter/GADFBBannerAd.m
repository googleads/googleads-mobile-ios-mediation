...
/// Converts ad size from Google Mobile Ads SDK to ad size interpreted by Meta Audience Network.
static FBAdSize GADFBAdSizeFromAdSize(GADAdSize gadAdSize, NSError *__autoreleasing *error) {
  ...
  if (error) {
    NSString *description = [NSString
        stringWithFormat:@"Invalid size for Meta Audience Network mediation adapter. Size: %@",
...
@implementation GADFBBannerAd {
  ...
  /// Banner ad obtained from Meta Audience Network.
  FBAdView *_bannerAd;
...
- (void)getBannerWithSize:(GADAdSize)adSize {
  ...
  if (!_bannerAd) {
    ...
    error = GADFBErrorWithCodeAndDescription(GADFBErrorAdObjectNil, description);
    [strongConnector adapter:strongAdapter didFailAd:error];
    return;
  }
